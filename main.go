...
func main() {
	...
	db_connection := "user=postgres dbname=chicago_business_intelligenc1 password=root host=/cloudsql/chicago-business-intelligenc1:us-central1:mypostgres1 sslmode=disable port = 5432"
...
func GetTaxiTrips(db *sql.DB) {
	...
	geocoder.ApiKey = "AIzaSyDmnz1QIpXCXvpKDZCnThp3hfE0qghByuo"
	...
	var url = "https://data.cityofchicago.org/resource/wrvz-psew.json?$limit=20"
...
func GetUnemploymentRates(db *sql.DB) {
	...
	// While doing unit-testing keep the limit value to 500
	// later you could change it to 1000, 2000, 10,000, etc.
	var url = "https://data.cityofchicago.org/resource/iqnk-2tcu.json?$limit=20"
...
func GetBuildingPermits(db *sql.DB) {
	...
	// While doing unit-testing keep the limit value to 500
	// later you could change it to 1000, 2000, 10,000, etc.
	var url = "https://data.cityofchicago.org/resource/building-permits.json?$limit=20"
